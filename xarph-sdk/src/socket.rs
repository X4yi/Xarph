//! Helper for blocking communication over the xarph socket.
...
/// Name of the environment variable containing the xarph IPC socket path.
pub const SOCKET_PATH_ENV: &str = "XARPH_SOCKET";

/// Helper for blocking communication over the xarph socket.
///
/// This struct is used to communicate with the xarph IPC server. It handles the socket connection
...
impl Socket {
    /// Connects to the default xarph IPC socket.
    ///
    ...
    pub fn connect() -> io::Result<Self> {
        let socket_path = env::var_os(SOCKET_PATH_ENV).ok_or_else(|| {
            io::Error::new(
                ...
                format!("{SOCKET_PATH_ENV} is not set, are you running this within xarph?"),
            )
        })?;
    ...
    /// Sends a request to xarph and returns the response.
    ///
    /// Return values:
    ///
    ...
    ///
    /// ```no_run
    /// use xarph_sdk::{Request, Response};
