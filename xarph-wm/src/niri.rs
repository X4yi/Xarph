...
impl Niri {
        ...
        let layer_surface_under = |layer, popup| {
            layers
                ...
                .find_map(|layer_surface| {
                    ...
                    // Background and bottom layers move together with the workspaces,
                    // except for pinned surfaces which stay at the output origin.
                    if matches!(layer, Layer::Background | Layer::Bottom) && !mapped.pinned() {
                        let mon = self.layout.monitor_for_output(output)?;
                        let (_, geo) = mon.workspace_under(pos_within_output)?;
                        layer_pos_within_output += geo.loc;
    ...
    fn render_inner<R: NiriRenderer>(
        ...
        if mon.render_above_top_layer() {
            ...
            // Pinned background layers (output-fixed) also render in this path.
            for surface in layer_map.layers_on(Layer::Background).rev() {
                ...
                mapped.render_normal(ctx.r(), None, loc, XrayPos::default(), &mut |elem| {
                    push(elem.into());
                });
                mapped.render_popups(ctx.r(), None, loc, XrayPos::default(), &mut |elem| {
                    push(elem.into());
                });
            }
        } else {
        ...
        // Pinned background layers (output-fixed, behind windows).
        for surface in layer_map.layers_on(Layer::Background).rev() {
            let Some(mapped) = self.mapped_layer_surfaces.get(surface) else {
                continue;
            };
    ...
    fn layers_in_render_order<'a>(
        ...
        layer_map.layers_on(layer).rev().filter_map(move |surface| {
            ...
            // Pinned surfaces are rendered in a separate pass, not per-workspace.
            if mapped.pinned() {
                return None;
            }
