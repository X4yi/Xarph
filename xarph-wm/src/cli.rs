...
pub struct Cli {
    /// Path to config file (default: `$XDG_CONFIG_HOME/xarph/config.kdl`).
    ///
    /// This can also be set with the `XARPH_CONFIG` environment variable. If both are set, the
...
#[derive(Subcommand)]
pub enum Sub {
    /// Communicate with the running xarph-wm instance.
    Msg {
        #[command(subcommand)]
        msg: Msg,
