...
const DEFAULT_LOG_FILTER: &str = "xarph_wm=debug,smithay::backend::renderer::gles=error";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.session {
        ...
        env::set_var("XDG_CURRENT_DESKTOP", "xarph");
    ...
    *CHILD_ENV.write().unwrap() = mem::take(&mut config.environment);
    ...
    if env::var_os("XARPH_DISABLE_SYSTEM_MANAGER_NOTIFY").is_none_or(|x| x != "1") {
...
fn env_config_path() -> Option<PathBuf> {
    env::var_os("XARPH_CONFIG")
        .filter(|x| !x.is_empty())
        .map(PathBuf::from)
}

fn default_config_path() -> Option<PathBuf> {
    let Some(dirs) = ProjectDirs::from("", "", "xarph") else {
...
fn system_config_path() -> PathBuf {
    PathBuf::from("/etc/xarph/config.kdl")
}
